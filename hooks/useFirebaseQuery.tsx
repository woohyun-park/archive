...
import { getTagsQueryByKeyword, getTagsQueryByUid } from "apis/firebase/queryNew/queryTags";
import { readAlarms, readPosts, readScraps, readTags, readUsers } from "apis/firebase";
...
export default function useFirebaseQuery(type: FirebaseQueryType) {
  ...
  const queryFnFor: IDict<(docs: QueryDocumentSnapshot<DocumentData>[]) => Promise<any>> = {
  ...
  const queryFor: IDict<(params: any) => FirebaseQueryStatus> = {
    "feed/posts": getPostsQueryByFollowings([...followings, curUid]),
    "profile/posts": getPostsQueryByUid(uid as string),
    "profile/scraps": getScrapsQueryByUid(uid as string),
    "profile/scraps/detail": getScrapsQueryByUidAndCont(uid as string, cont as string),
    "profile/tags": getTagsQueryByUid(uid as string),
    "profile/tags/detail": getPostsQueryByUidAndTag(uid as string, tag as string),
